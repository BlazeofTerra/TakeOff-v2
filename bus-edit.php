...
if($stmt = mysqli_prepare($dbca, "SELECT name, contact, email FROM accounts WHERE id = ?")) {
...
if($stmt = mysqli_prepare($dbcg, "SELECT business_name, website, contact, email, fax, address, postcode, country, content, image_1, category_1, 
										 category_2, category_3, category_4, category_5, subcategory_1, subcategory_2, subcategory_3, subcategory_4,
										 subcategory_5, image_2, image_3, image_4, image_5, image_6, image_7 FROM posts WHERE userid = ? AND type = 1")) {
	mysqli_stmt_bind_param($stmt, "i", $account_id);
	mysqli_stmt_execute($stmt);
	$meta = $stmt->result_metadata();
...
if(!isset($_SESSION['busEdit'])) {
	$_SESSION['busEdit'] = array("image_1"=>$business['image_1'], 
								 "image_2"=>$business['image_2'], 
								 "image_3"=>$business['image_3'], 
								 "image_4"=>$business['image_4'], 
								 "image_5"=>$business['image_5'], 
								 "image_6"=>$business['image_6'], 
								 "image_7"=>$business['image_7']);
}
...
if(isset($_POST['name'])) {
	for($i = 1; $i < 8; $i++) {
		${"image_".$i} = $_SESSION['busEdit']['image_'.$i];
		${"image_".$i."_aspect"} = $_SESSION['busEdit']['image_'.$i];
	}
	...
	if($stmt = mysqli_prepare($dbca, "UPDATE accounts SET name = ?, contact = ? WHERE id = ?")) {
	    ...
	    mysqli_stmt_bind_param($stmt, "ssi", $_POST['name'], $_POST['contactNumber'], $account_id);
	...
	if($stmt = mysqli_prepare($dbcg, 
	    ...
	    mysqli_stmt_bind_param($stmt,
	    	"ssssssssssssssssssssssssssi",
	   		$_POST['businessName'], 
	   		$_POST['website'], 
	   		$_POST['businessContactNumber'],
	   		$_POST['businessEmail'],
	   		$_POST['businessAddress'],
	   		$_POST['businessPostcode'],
	   		$_POST['country'],
	   		$_POST['description'],
	   		$image_1,
	   		$_POST['category_1'],
...
for($i = 1; $i < 8; $i++) {
	if($business['image_'.$i] != "") {
		${"image_".$i} = $business['image_'.$i];
		${"image_".$i."_aspect"} = substr($business['image_'.$i], 0);
	...
	<body>
		<div id="outerWrapper">
			<div id="wrapper">
				...
				<form id="formWrapper" action="bus-edit.php" method="post">
					<h1>Business Registration</h1>
					
					<div id="errorHolder">
						<p id="firstNameError" style="display: none;">Your first name is required</p>
						<p id="surnameError" style="display: none;">Your surname name is required</p>
					...
					<div class="inputContainer">
						...
						<div class="inputRow"><label for="firstName">Your name:</label><input id="name" type="text" name="name" value="<?php echo $account['name']; ?>" /></div>
						<div class="inputRow"><label for="contactNumber">Contact Number:</label><input id="contactNumber" type="text" name="contactNumber" value="<?php echo $account['contact']; ?>" /></div>
					</div>
					
					<div class="inputContainer">
						<h2>Your Business Information</h2>
						<div class="inputRow"><label for="businessName">Business Name:</label><input id="businessName" type="text" name="businessName" value="<?php echo $business['business_name']; ?>" /></div>
						<div class="inputRow"><label for="businessContactNumber">Business Contact Number:</label><input id="businessContactNumber" type="text" name="businessContactNumber" value="<?php echo $business['contact']; ?>" /></div>
						<div class="inputRow"><label for="businessEmail">Business Email:</label><input id="businessEmail" type="email" name="businessEmail" value="<?php echo $business['email']; ?>" /></div>
						<div class="inputRow"><label for="businessEmail">Business address:</label><input id="businessAddress" type="text" name="businessAddress" value="<?php echo $business['address']; ?>" /></div>
						<div class="inputRow"><label for="businessEmail">Business postcode:</label><input id="businessPostcode" type="text" name="businessPostcode" value="<?php echo $business['postcode']; ?>" /></div>
						<div class="inputRow"><label for="website">Website:</label><input id="website" type="url" name="website" value="<?php echo $business['website']; ?>" /></div>
						<div class="inputRow"><label for="countie">Country:</label>
							<select name="country">
								<?php foreach ($countries as $key => $value) { ?>
									<option value="<?php echo $value['name']; ?>"<?php if($business['country'] == $value['name']) { echo " selected"; } ?>><?php echo $value['name']; ?></option>
								<?php } ?>
							</select>
						</div>
					</div>
							...
							<div class="categoryHolder">
								<div class="categories">
									<select id="category_1" class="categoryPicker" name="category_1">
										...
										<?php if($business['category_1'] != 0) {
											foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"<?php if($business['category_1'] == $value['id']) { echo " selected"; } ?>><?php echo $value['label']; ?></option>
											<?php }
										} else { ?>
								...
								<div class="subCategories">
									<select id="subCategory_1" name="subCategory_1">
										<?php if($business['subcategory_1'] != 0) {
											subCat($dbcg, $business['category_1'], $business['subcategory_1']);
										} else { ?>
											<option value="0">--</option>
										<?php } ?>
									</select>
							...
							<div class="categoryHolder">
								<div class="categories">
									<select id="category_2" class="categoryPicker" name="category_2">
										<option value="0">--</option>
										<?php if($business['category_2'] != 0) {
											foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"<?php if($business['category_2'] == $value['id']) { echo " selected"; } ?>><?php echo $value['label']; ?></option>
											<?php }
										} else { ?>
											<?php foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"><?php echo $value['label']; ?></option>
								...
								<div class="subCategories">
									<select id="subCategory_2" name="subCategory_2">
										<?php if($business['subcategory_2'] != 0) {
											subCat($dbcg, $business['category_2'], $business['subcategory_2']);
										} else { ?>
											<option value="0">--</option>
										<?php } ?>
									</select>
							...
							<div class="categoryHolder">
								<div class="categories">
									<select id="category_3" class="categoryPicker" name="category_3">
										<option value="0">--</option>
										<?php if($business['category_3'] != 0) {
											foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"<?php if($business['category_3'] == $value['id']) { echo " selected"; } ?>><?php echo $value['label']; ?></option>
											<?php }
										} else { ?>
											<?php foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"><?php echo $value['label']; ?></option>
								...
								<div class="subCategories">
									<select id="subCategory_3" name="subCategory_3">
										<?php if($business['subCategory_3'] != 0) {
											subCat($dbcg, $business['category_3'], $business['subCategory_3']);
										} else { ?>
											<option value="0">--</option>
										<?php } ?>
									</select>
							...
							<div class="categoryHolder">
								<div class="categories">
									<select id="category_4" class="categoryPicker" name="category_4">
										<option value="0">--</option>
										<?php if($business['category_4'] != 0) {
											foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"<?php if($business['category_4'] == $value['id']) { echo " selected"; } ?>><?php echo $value['label']; ?></option>
											<?php }
										} else { ?>
											<?php foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"><?php echo $value['label']; ?></option>
								...
								<div class="subCategories">
									<select id="subCategory_4" name="subCategory_4">
										<?php if($business['subCategory_4'] != 0) {
											subCat($dbcg, $business['category_4'], $business['subCategory_4']);
										} else { ?>
											<option value="0">--</option>
										<?php } ?>
									</select>
							...
							<div class="categoryHolder">
								<div class="categories">
									<select id="category_5" class="categoryPicker" name="category_5">
										<option value="0">--</option>
										<?php if($business['category_5'] != 0) {
											foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"<?php if($business['category_5'] == $value['id']) { echo " selected"; } ?>><?php echo $value['label']; ?></option>
											<?php }
										} else { ?>
											<?php foreach ($categories as $key => $value) { ?>
												<option value="<?php echo $value['id']; ?>"><?php echo $value['label']; ?></option>
								...
								<div class="subCategories">
									<select id="subCategory_5" name="subCategory_5">
										<?php if($business['subCategory_5'] != 0) {
											subCat($dbcg, $business['category_5'], $business['subCategory_5']);
										} else { ?>
											<option value="0">--</option>
										<?php } ?>
									</select>
					...
					<div class="inputContainer">
						...
						<div class="descHolder">
							<textarea name="description"><?php echo $business['content']; ?></textarea>
						</div>
					</div>
