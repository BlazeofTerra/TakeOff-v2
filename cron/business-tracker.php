...
if($stmt = mysqli_prepare($dbcg, "SELECT category_1, category_2, category_3, category_4, category_5 FROM posts WHERE type = 1")) {
	mysqli_stmt_execute($stmt);
	$meta = $stmt->result_metadata();
...
foreach($categories as $key => $value) {
	if($stmt = mysqli_prepare($dbcg,
		"UPDATE category SET number_businesses = ? WHERE id = ?")) {
